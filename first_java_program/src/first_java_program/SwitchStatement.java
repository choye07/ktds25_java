...
public class SwitchStatement {
	public static void main(String[]args) {
		...
		//라면 끓이기 절차를 출력해보기.
		switch(step) { //소괄호에는 어디까지 했는지.
		case 1:
			System.out.println("1. 수납장을 손잡이를 잡아 문을 연다.");
		case 2:
			System.out.println("2. 수납장에서 냄비 손잡이를 잡아 꺼낸다.");
		case 3:
			System.out.println("3. 냄비를 세척해야할지 판단한다(if)");
		case 4:
			...
			if(randomNumber % 2 == 0) {				
				...
				System.out.println("4. 냄비를 씻다가 떨어트려 깨져버렸습니다. 라면을 끓일 수가 없어요.");
				break;
			}
			else {
				System.out.print("("+randomNumber+") ");
		...
		*/
		...
		// Swich를 이용해서 변수에 값을 할당한다.
		// Java 13 switch
		// 회원가입을 받을 때, 사용자가 등록할 수 없는 아이디 목
		// 관리자들이 사용하는 아이디
...
/*		
			...
			case"superuser":
			case "administrator":
				yield "관리자";// 관리자 값이 할당이 되고, yield가 switch를 끊어준다.
			case "master":
			case "operator":
			case "system":
				yield "운영자";
...
*/
		...
		case "master", "operator","system"-> "운영자";
		default -> "일반 사용자";
		};
